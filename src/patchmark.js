		...
		patterns = {
			...
			blocks: {
				...
				paragraph : {
					...
					func : function(m) {
						var html = m;
						if (html.search(/\S/) < 0) return "";
						if (html.search(/^\|\|PMB\d+PMB\|\|/) > -1) return html;
						if (html.search(/^[ ]{0,3}<(h\d|div|blockquote|pre|ul|ol)>[\s\S]+?(<\/\1>)?/) > -1) return html;
						if (html.search(/^[ ]{0,3}<hr \/>/) > -1) return html;
